...
//
// ⚠️ Why this boilerplate exists and cannot be reduced (issue #267):
//
// Each `Operations.<op>.Output` is a distinct `@frozen` enum generated by
// swift-openapi-generator. The enum cases carry operation-specific associated
// values (e.g. `case unauthorized(Operations.get_card.Output.Unauthorized)`),
// making every Output type structurally unique.
//
// Swift protocols cannot abstract over enum case patterns — there is no way
// to express "this type has a `.ok(T)` case" in a protocol. This means:
//
// 1. **Protocol per pattern** won't work: you'd still need to implement
//    computed properties (e.g. `var okBody: T?`) on each type — same or
//    more boilerplate than the current switch.
//
// 2. **Single generic function** is impossible: the switch must be exhaustive
//    over each enum's specific cases, and they differ across operations.
//
// 3. **Macros** could generate these extensions, but are excluded from scope.
//
// 4. **Code generation** (e.g. Sourcery/gyb) could template these, but adds
//    a build-tool dependency for marginal gain over 67 simple extensions.
//
// The 67 extensions below (one per API operation) are the minimal correct
// representation. Each is ~6 lines and purely mechanical.
// When adding a new API operation, copy the nearest pattern and adjust cases.
