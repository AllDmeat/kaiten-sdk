...
- **FR-015**: OpenAPI-спека MUST содержать только эндпоинты, которые реально используются в SDK. Пока у Kaiten нет официальной OpenAPI-спеки, мы поддерживаем минимальную hand-crafted спеку — только используемые read-операции. Это минимизирует ручную валидацию. Когда Kaiten предоставит официальную спеку, можно перейти на неё целиком (включая неиспользуемые методы).
- **FR-016**: OpenAPI-спека (`openapi/kaiten.yaml`) собирается **вручную** — у Kaiten нет публичной OpenAPI-спецификации. Спека MUST точно зеркалить реальное Kaiten API по документации (https://developers.kaiten.ru):
  - **1:1 с документацией** — каждый эндпоинт, каждое поле, каждый тип должен соответствовать документации. Никаких упрощений или объединений ради DRY.
  - **Разные ответы = разные схемы** — если два эндпоинта возвращают похожие, но не идентичные структуры, в спеке MUST быть отдельные схемы (пример: `Board` vs `BoardInSpace`).
  - **Nullable и required строго по докам** — `null | type` в документации = nullable в спеке, просто `type` = required. НЕ определять по эмпирическим данным.
  - **Перепроверка обязательна** — при любом изменении спеки сверяться с документацией Kaiten. Проверять все поля, типы, nullable/required. Гайд по парсингу документации: [docs/kaiten-docs-parsing.md](../../docs/kaiten-docs-parsing.md).
